...
import io.quarkiverse.jef.java.embedded.framework.linux.core.types.ByteReference;
...
public class BluetoothJna extends Bluetooth {
    ...
    @Override
    public int hci_le_set_scan_parameters(int dev_id, byte scan_type, short interval, short window, byte own_type, byte filter,
            int to) {
    ...
    public void hci_filter_set_ptype(int t, HciFilter f) {
        ...
        hci_set_bit(
                (t == HCI_VENDOR_PKT) ? 0 : (t & HCI_FLT_TYPE_BITS),
                Pointer.nativeValue(ff.getPointer()) + ff.offset("type_mask"));
        ff.fill(f);
    }
    ...
    static class Delegate {
        ...
        public static native int hci_inquiry(int dev_id, int len, int max_rsp, ByteByReference lap, PointerByReference ii,
                NativeLong flags);

        public static native void hci_close_dev(int dd);

        public static native int hci_le_set_scan_parameters(int dev_id, byte scan_type, short interval, short window,
                byte own_type, byte filter, int to);
