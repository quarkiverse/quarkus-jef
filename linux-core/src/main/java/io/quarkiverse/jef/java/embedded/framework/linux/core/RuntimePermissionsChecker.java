    ...
    private static void validate() {
        ...
        if (ss.getuid() == ROOT_USER_ID && ss.geteuid() == ROOT_USER_ID) {
            ...
            if (!groupok) {
                ...
                log.log(Level.SEVERE, write("ERROR: Current user don't required groups. Please run command 'sudo usermod -a -G "
                        + str + " " + ss.getLogin() + "'"));
                write("");
            } else {
             ...
             * log.log(Level.SEVERE, write("ERROR: Please run command 'sudo setcap cap_sys_rawio=ep " + path +
             * "' before execution."));
             * write("");
             * error = true;
             * }
